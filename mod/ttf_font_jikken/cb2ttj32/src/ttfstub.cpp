...
#else
unsigned char ptn_2byte[189] =
"!I¡¨¡­"C"H¡®¡¦!]!^¡¯¡Ï!A¡Ğ!D¡ş¢¯¢°¢±¢²¢³¢´¢µ¢¶¢·¢¸!G!F¡Õ¡×¡Ö!H"I¢Ï¢Ğ¢Ñ¢Ò¢Ó¢Ô¢Õ¢Ö¢×¢Ø¢Ù¢Ú¢Û¢Ü¢İ¢Ş¢ß¢à¢á¢â¢ã¢ä¢å¢æ¢ç¢è!e"@!f!s¡Å¡¥¢é¢ê¢ë¢ì¢í¢î¢ï¢ğ¢ñ¢ò¢ó¢ô¢õ¢ö¢÷¢ø¢ù¢ú¢û¢ü¢ı¢ş#@#A#B#C!a!U!b¡ã";
#endif
...
void gene_cmap()
     ...
     for (fnum = EachFont*31, base = 0, i = 2;
         ...
         {
         sub_ary[i].fCod   = 64;
         sub_ary[i].numEnt = (fnum > EachFont)?EachSeg:((fnum <= 63)?fnum:(fnum+EachOff));	//‚±‚ÌãˆÊƒoƒCƒg‚Éƒ}ƒbƒv‚³‚ê‚Ä‚é•¶š‚Ì”
...
void glyf_engfnt(bigfirst &glyf, short i)
     {
     // ”¼Šp•¶š‚Ì€”õ
     glyf.addshort((unsigned short)-1);		//numberObContours=-1 .. ‡¬Glyph‹Lq
     glyf.addshort(0);						//cƒCƒ“ƒfƒbƒNƒX
     glyf.addshort(-1*DESCENT);				//xƒIƒvƒZƒbƒg
     glyf.addshort(XSIZE/2); 				//xƒIƒtƒZƒbƒg//hmtx define the kerning, not here
     glyf.addshort(YSIZE-DESCENT);			//yƒIƒtƒZƒbƒg
     glyf.addshort(0x53);					//•ÏŒ`ƒIƒvƒVƒ‡ƒ“
#ifdef SJIS
     /* ptn_2byte is in EUC */
