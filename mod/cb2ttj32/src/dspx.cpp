...
extern "C"
     {
     void curve(short x0,short y0,short x1,short y1,short x2,short y2,short x3,short y3);
     }

void main(short argc, char *argv[])
     {
     if (argc < 3) { cout << "Usage:DSPX bitmapfile splinefile\n"; return; }
     ...
     fclose(fn);
     short lx, ly, x1, y1, x2, y2, x0, y0, flag = 0;
     _setcolor(12);
     for (;;)
         {
         ...
         if (flag) curve(lx,ly,x1,y1,x2,y2,x0,y0);
         else _lineto(x0,y0);
         }
     fclose(fx);
     _getch();
...
void curve(short x0,short y0,short x1,short y1,short x2,short y2,short x3,short y3)
     ...
     for (t = 0, dt = 1/(float)__max(abs(x3-x1),abs(y3-y1)); (dt > 0) && (t < 1); t+=dt)
         _lineto((short)((x2+x3)*t*t/2+2*t*(1-t)*x2+(x1+x2)*(t-1)*(t-1)/2),
                 (short)((y2+y3)*t*t/2+2*t*(1-t)*y2+(y1+y2)*(t-1)*(t-1)/2) );
     _lineto(x3,y3);
//     _lineto(x2,y2);
     }
