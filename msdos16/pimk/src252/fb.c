/*
   ...
   このソースは元配布物に対し、Pimk用にいくらか手を加えています。
   （かってにプログラムを終了されるとこまるので、exit() -> Exit() に変更
   　したり、とか）
   これを流用しようというかたは、元配布のものを入手したほうがいいです。
	（ぼくは NIFTY fgalav より入手しました）
														   てんかﾐ☆
 */
/*---------------------------------------------------------------------------*/
/*
...
>う～～～～誰か～～～ＳＦのスクロールのテストの結果を教えてくれ～～(;_;)
       ...
       で、例のソースで2048-80をレジスタに叩き込むと16ドット上に上がるん
       ...
       で、普段アクセス出来ない一番下に隠れている48キャラ分ですが、どーやら
