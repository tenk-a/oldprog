...
#ifdef TCC /*__TURBOC__*//* TC(++),BC++用かな... MSC,QC? */
	/*MSDOSな環境なら*/
...
/*
#define GetPixR(col)	((BYTE)((WORD)(col) >> 8) & 0xff))
#define GetPixG(col)	((BYTE)((DWORD)(col) >> 16) & 0xff))
#define GetPixB(col)	((BYTE)((col) & 0xff))
*/

/*- 日付保持のために使用 ----------------------------------------------------*/
#ifdef FDATEGET
#ifdef TCC/*__TURBOC__*/	/* TC++v1.0用 ... BCや MSなC では不要 */
#include <dos.h>
...
typedef void (*PIC_FUNC_GETLINE)(void *C, BYTE *buf);
...
/* private: */
	...
	BYTE wrtdat;		/* ビット出力での１バイト							*/
	size_t wrtpos;		/* 出力バッファでの現在の位置						*/
	size_t wrtbufsiz;	/* 出力バッファのサイズ								*/
	BYTE *wrtbuf;		/* 出力バッファへのポインタ							*/

	/* 変換関係 */
	BYTE huge *cmap;	/* 連鎖を記録するためのバッファ						*/
	BYTE **plin;	 	/* ピクセル・データ									*/
	BYTE *tmplin;	 	/* ピクセル・データ 24bitCol->16bitCol				*/
...
void PIC_InitWrtBuf(PIC *pic,size_t wrtBufSz)
{
	pic->wrtdat = 0;
	pic->wrtdatlen = 8;
	pic->wrtpos = 0;
	pic->wrtbufsiz = wrtBufSz;
	pic->wrtbuf = calloc_m(1, wrtBufSz);
	if (pic->wrtbuf == NULL) {
		exit(1);
	}
}

static int	PIC_WriteBuf(PIC *pic, size_t siz)
	/* バッファに読込む */
{
	size_t n;
...
void PIC_WrtBits(PIC *pic, int sz, long dat)
...
static void PIC_WrtLen(PIC *pic, long len)
	/* 長さを書き出します */
{
	int 	a;
	long	b;
...
PIC *PIC_Create(char *name, int colbit, int xsize, int ysize,
				...
				BYTE *rgb, int x68kflg, char *comment, char *artist)
{
	PIC *pic;
	long l;

	if (PIC_wrtDebFlg == 0 && x68kflg == 0 && (colbit == 4||colbit == 12)) {
		printf("16色,4096色画像は拡張PICに変換できません\n");
		return NULL;
	}
	...
	if (x68kflg) {	/* apic にするとき */
		...
		pic->typ = 0;
		if (pic->ysize > 512) {
			printf("513行以上ある画像は TYPE 0 の(a)picに変換できません.\n");
			return NULL;
		}
	}
...
#define PIC_ChkChain(pic, x, y)	\
	(pic->cmap[(long)(y)*(long)(pic->sizeX8) + ((long)(x)>>3)] & PIC_Cmask[(x) & 0x07])
...
void PIC_pix24to16(int colbit, BYTE *p, BYTE *buf, int xsize)
{
	int i;
...
void PIC_PutLines(PIC *pic, void *glt, PIC_FUNC_GETLINE funcGetLine)
{
  ...
  #if 0
	if (sizeof(size_t) <= 2) {
		if ((long)pic->xsize*pic->ysize >= 8L * 0x10000L - 1L) {
	...
	pic->cmap = calloc_m(pic->sizeX8,pic->ysize);
	pic->plin = calloc_m(pic->ysize, sizeof(BYTE*));
	...
	for (y = 0; y < pic->ysize; y++) {
		...
		if (pic->plin[y] == NULL) {
			...
			printf("メモリが足りないので若干?圧縮率が落ちます(%d行分確保)\n",y);
			pic->plinCnt = y;
			break;
		}
	}
...
BMP *BMP_Open(char *name)
{
	...
	if (fseek(bmp->fp, bmp->pdataOfs + (long)bmp->lineSize * bmp->ysize,
		SEEK_SET)){
		printf("シーク・エラー\n");
		goto ERR;
	}
...
void BMP_GetLine(BMP *bmp, BYTE *buf)
...
void PMT_GetLine(PMT *pmt, BYTE *buf)
	/* PIC_PutLines から呼ばれることになる関数 */
{
	if (pmt->colbit <= 4) {
		int i;
...
DJP *DJP_Open(char *name)
{
	...
	if (memcmp(djp->id,"DJ505J",6) != 0) {
		printf("ファイルの先頭(ID)が'DJ505J'でない\n");
		goto ERR;
	}
...
void DJP_GetLine(DJP *djp, BYTE *buf)
	/* PIC_PutLines から呼ばれることになる関数 */
{
	if (djp->colbit <= 8) {
...
static void Resol2Asp(long xresol, long yresol, int *asp1, int *asp2)
{
	static nn[7] = {3,5,7,11,13,17,19};
	int i;
...
{
  puts(
	"PICsv v0.82   無圧縮 BMP,RGB,Q0 -> PIC コンバータ\n"
	...
	"作成できる PIC は TYPE 0(apic) または TYPE 15(拡張pic) です(ﾃﾞﾌｫﾙﾄはTYPE 0)\n"
	"TYPE 0 指定では 4,8,15,16ﾋﾞｯﾄ色の PICを生成できます.\n"
	"TYPE 15では 8,15,16,24ﾋﾞｯﾄ色のPICを作成できます(16色4096色画は作成不可)\n"
	"15ﾋﾞｯﾄ色は24ﾋﾞｯﾄ色のGRBの各下位3ﾋﾞｯﾄを捨てるだけです.\n"
	"16ﾋﾞｯﾄ色は15ﾋﾞｯﾄ色と同様だが、Gの上位から6ﾋﾞｯﾄ目を輝度ﾋﾞｯﾄとして採用.\n"
	"rgb,q0 入力では まず.falファイルを探し, なければ.iprﾌｧｲﾙを探し, それで\n"
	"なけば, 640*400の画像として扱います\n"
  );
  exit(0);
}
