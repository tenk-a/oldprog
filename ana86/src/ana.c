...
#define TITLE	"ana86 開発途上版(L1) Ver. 0.19 (" __DATE__ ")  by M.Kitamura"
...
void
ChgDirExt(byte onam[], byte *inam, byte *dir, byte *ext)
{
	byte *np;
	byte *s;
	byte *p;
...
OutExtrnName(St_t_fp sp)
{
	...
	if (sp->v.tok == T_MODULE && (/*Opt_partFlg || */ sp->v.grp != Mod_sp))
		OutExtrnName(sp->v.grp->v.st);
	if (sp->v.flg2 & FL_USE) {
		sp->v.flg2 &= (~FL_USE);
		fprintf(Out_file, (Opt_partFlg ? "global %s" : "extrn %s")
			,Str_StName(sp));
		if (Opt_r86 == 0) {
...
static void
OutSegBgn(St_t_fp sp)
{
	static byte *align[] = {"byte","word","para","page"};
	static byte *combi[] = {"public","private","common","stack","at"};
	static byte *lsiseg[2] = {"%s\tcseg %s %s ", "%s\tdseg %s %s "};
	static byte *lsicombi[] = {"","","","stack","at"};
...
DeclSeg(void)
{
 ...
 J0:
	switch (Sym_Get()) {
	case I_PUBLIC:  sp->c.combi = 0;	goto J1;
	case I_PRIVATE: sp->c.combi = 1;	goto J1;
	case I_COMMON:  sp->c.combi = 2;	goto J1;
	case I_STACK:	sp->c.combi = 3;
 J1:
		Sym_Get();
		break;
	case I_COMMA:
...
static int
DeclImportSub(byte *fnam,St_t_fp sp,int dirflg)
{
	...
	while (Ana_err == 0) {
		...
		if (Sym_tok == I_CR || Sym_tok == I_SMCLN) {
			...
			if (Sym_Get() != T_IDENT) {
				...
				if (Decl_defFlg) {
					if ((oStart_sp = Sym_NameNew(Sym_name,T_STARTLBL,Ana_mode))
						== NULL)
						goto ENDF;
				}
				Sym_Get();
