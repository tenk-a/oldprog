...
Et_New(void)
{
	...
	if (Et_enpTop->e.next == NULL) {
	  #if 0
		if ((Et_enpTop->e.next = Et_Init0()) == NULL) return NULL;
	  #else
...
St_New(void)
{
	...
	if (St_enpTop->c.next == NULL) {
	  #if 0
		if ((St_enpTop->c.next = St_Init0()) == NULL) return NULL;
	  #else
...
#if 0
void
St_Chk1(St_t_fp sp, int lr, int n)
{
	int i;
...
int
St_Chk(St_t_fp sp, int lr, int n, int nn)
{
	int i,j,k;
...
#if 0
void
StrKai(St_t_fp pp,St_t_fp p,byte far *str,word n,word nl,word f,word a)
{
...
St_t_fp
St_Ins(byte_fp name, St_t_fp far* root)
{
...
St_t_fp
St_Search(byte_fp name,St_t_fp root)
{
	int  a;
	St_t_fp sp;
